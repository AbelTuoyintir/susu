        ...
        if ($data['status'] && $data['data']['status'] === 'success') {
            ...
            if ($paymentType === 'contribution') {
                ...
                if ($book->user_id != $userId) {
                    Log::error("Book ownership mismatch for book $bookId. Expected owner: $book->user_id, Paid by: $userId");
                    return response()->json(['status' => false, 'message' => 'Unauthorized payment'], 403);
                }
                ...
                if ($loan) {
                    if ($loan->user_id != $userId) {
                        Log::error("Loan ownership mismatch for loan $loanId. Expected owner: $loan->user_id, Paid by: $userId");
                        return response()->json(['status' => false, 'message' => 'Unauthorized payment'], 403);
                    }
