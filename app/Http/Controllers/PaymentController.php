...
class PaymentController extends Controller
{
    public function verify(Request $request)
    {
        ...
        if ($data['status'] && $data['data']['status'] === 'success') {
            try {
                return \Illuminate\Support\Facades\DB::transaction(function () use ($data, $reference) {
                    ...
                    // Check if payment already processed (inside transaction for safety)
                    if (Payment::where('transaction_id', $reference)->lockForUpdate()->exists()) {
                        return response()->json(['status' => true, 'message' => 'Already processed']);
                    }

                    if ($paymentType === 'contribution') {
                        $bookId = $metadata['book_id'];
                        // Use lockForUpdate on the book to prevent concurrent contribution entries for the same book
                        $book = Book::where('id', $bookId)->lockForUpdate()->first();
                        if (!$book) {
                            return response()->json(['status' => false, 'message' => 'Invalid book'], 400);
                        }

                        // Refine week_number determination with a lock
                        ...
                        // Use lockForUpdate on the loan record
                        $loan = Loan::where('id', $loanId)->lockForUpdate()->first();
                        if ($loan) {
                            // SECURITY: Ensure user owns the loan
                            if ($loan->user_id != $userId) {
                                Log::error("Unauthorized loan payment attempt for loan $loanId by user $userId");
                ...
                Log::error("Error processing Paystack verification: " . $e->getMessage());
                return response()->json(['status' => false, 'message' => 'Internal server error during verification'], 500);
            }
        }
